...
boot:
	...
	ldh r8, #one_ms_counter
	ldl r8, #one_ms_counter
	ldh r6, #61h
	ldl r6, #A8h
	st  r6, r8, r0			    ; one_ms_counter <- 25e3
	
	
	ldh r8, #one_s_counter
	ldl r8, #one_s_counter
	ldh r6, #03h
	ldl r6, #E8h
	st  r6, r8, r0			;one_s_counter <- 1000
...
main:
    ...
    jsrd #srt_subtract_cycles
    
    jmpd #main

; Subroutines
srt_subtract_cycles:
    ; Objective: to decrement one_ms and one_s counters according to processor usage
    ; Argument: r1 as number of instructions used
    ; Return: r3 as one_ms_counter state and r4 as one_s_counter state (1 if still positive)
    push r0
    ...
    ldh r6, #one_ms_counter    ;
    ldl r6, #one_ms_counter    ;
    ld r7, r6, r0                   ; r7 <- mem[one_ms_counter]
    subi r7, #28                    ; compensate for the 14 instructions of the subroutine
    sub r7, r7, r5                  ; r7 <- mem[one_ms_counter] - arg*2
    jmpzd #cyc_zero_or_neg          ;
    jmpnd #cyc_zero_or_neg          ; if one_ms_counter is positive:
    addi r3, #1                     ;   ret0 <- 1
    st r7, r6, r0                   ;   mem[one_ms_counter] <- mem[one_ms_counter] - arg*2
    jmpd #condition_end     
    cyc_zero_or_neg:                ; else
    ldh r7, #61h                    ;
    ldl r7, #A8h                    ;
    st r7, r6, r0                   ;   reset one_ms_counter
        
    ldh r6, #one_s_counter           ;
    ldl r6, #one_s_counter           ;
    ld r7, r6, r0                   ; 
    subi r7, #1                     ;   r7 <- mem[one_s_counter] - 1
    jmpzd #mili_zero_or_neg         ;       
    jmpnd #mili_zero_or_neg         ;   if one_s_counter is positive:
    addi r4, #1                     ;       ret1 <- 1
    st r7, r6, r0                   ;       mem[one_s_counter] <- mem[one_s_counter] - 1
    jmpd #condition_end     
    mili_zero_or_neg:               ;   else
    ldh r7, #03h                    ;
    ldl r7, #E8h                    ;
    st r7, r6, r0                   ;       reset one_s_counter
    condition_end:
...
.data
	...
	one_ms_counter:		    db #61A8h												; 25e3 instructions result in 4ms
	one_s_counter:			db #03E8h												; 1000 times of 4 ms counter resets results in a second
.enddata